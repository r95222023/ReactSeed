  ...
  render() {
    ...
    const loginPanel = (
      <div className={s.container}>
        ...
        <div className={s.formGroup}>
          <button className={s.facebook} onClick={loginWithProvider('facebook')}>
            <svg
              className={s.icon}
              width="30"
              height="30"
        ...
        <div className={s.formGroup}>
          <button className={s.google} onClick={loginWithProvider('google')}>
            <svg
              className={s.icon}
              width="30"
              height="30"
        ...
        <div className={s.formGroup}>
          <button className={s.twitter} href="/login/twitter">
            <svg
              className={s.icon}
              width="30"
              height="30"
    ...
    return (
      <div className={s.root}>
        {!auth.isEmpty ? userPanel : loginPanel}
      </div>
    );
  }
}
...
const mapStateToProps = (state) => {
  const { title } = state.firebase.data.test || {};
...
const mapDispatchToProps = dispatch => ({
  loginWithProvider: provider => () => {
    dispatch(login({ provider, type: 'popup' }));
  },
  _logout: () => {
    dispatch(logout());
  },
